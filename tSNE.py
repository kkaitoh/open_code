...
def optimized_function(X, trial):
    ...
    params = {
        "perplexity": trial.suggest_uniform("perplexity", 10, 1000),
        "learning_rate": trial.suggest_uniform("learning_rate", 0.1, 30),
        "n_iter": trial.suggest_int("n_iter", 500, 3000)
    }
    
    # tSNE を実行
    tsne = TSNE(**params, random_state = 0)
    X_tsne = tsne.fit_transform(X)

    # k3n error が小さくなるように探索する
    return k3n_error(X, X_tsne)
...
tsne = TSNE(**best_params, random_state = 0)
X_tsne = tsne.fit_transform(X_scaled)


k3n_error(X_scaled, X_tsne_plane)
